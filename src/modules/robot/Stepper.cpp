...
void Stepper::on_block_begin(void *argument)
{
    ...
    if(block->millimeters > 0.0F && (block->steps[ALPHA_STEPPER] > 0 || block->steps[BETA_STEPPER] > 0 || block->steps[GAMMA_STEPPER] > 0 || block->steps[DELTA_STEPPER] > 0) ) {
        block->take();
    ...
    if( block->steps[DELTA_STEPPER] > 0 ) {
        THEKERNEL->robot->delta_stepper_motor->move( block->direction_bits[DELTA_STEPPER], block->steps[DELTA_STEPPER])->set_moved_last_block(true);
        if(this->main_stepper == nullptr || THEKERNEL->robot->delta_stepper_motor->get_steps_to_move() > this->main_stepper->get_steps_to_move())
            this->main_stepper = THEKERNEL->robot->delta_stepper_motor;
    }else{
        THEKERNEL->robot->delta_stepper_motor->set_moved_last_block(false);
    }
...
uint32_t Stepper::stepper_motor_finished_move(uint32_t dummy)
{
    ...
    if( THEKERNEL->robot->alpha_stepper_motor->moving || THEKERNEL->robot->beta_stepper_motor->moving || THEKERNEL->robot->gamma_stepper_motor->moving  || THEKERNEL->robot->delta_stepper_motor->moving ) {
        return 0;
    }
...
void Stepper::set_step_events_per_second( float steps_per_second )
{
    ...
    if( THEKERNEL->robot->delta_stepper_motor->moving ) {
        THEKERNEL->robot->delta_stepper_motor->set_speed(isps * this->current_block->steps[DELTA_STEPPER]);
    }
